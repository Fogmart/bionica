(function ($, window, document, undefined) {
	...
	var defaultOptions = {
		...
		withParents: true, // Получить объекты вместе с родительскими
		...
		valFormat: function (obj, frmt) {
			...
			if (obj.parents) {
				...
				return $.kladr.buildAddressShort(objs);
			}
	...
	$.kladr = $.extend($.kladr, {
		...
		buildAddressShort: function (objs) {
			var lastIds = [],
				address = '',
				zip = '';
...
// console.log(label)
			...
			function setCurrent(obj) {
				...
				if ($input.attr("id") == 'city') $input.val(get('valFormat')(obj))
