...
if($_SESSION['admin']=="1")
$mysqli->query("DELETE FROM `notifications` WHERE `id` = ".$_POST['del'].";");
else	
...
// Add data
<?
$visit_1 = $mysqli->query('select id from visit where day(OrderDate) = day(now())-6')->num_rows;
$date_1 = new DateTime('-6 days'); $date_1 = $date_1->format('Y-m-d');
$visit_2 = $mysqli->query('select id from visit where day(OrderDate) = day(now())-5')->num_rows;
$date_2 = new DateTime('-5 days'); $date_2 = $date_2->format('Y-m-d');
$visit_3 = $mysqli->query('select id from visit where day(OrderDate) = day(now())-4')->num_rows;
$date_3 = new DateTime('-4 days'); $date_3 = $date_3->format('Y-m-d');
$visit_4 = $mysqli->query('select id from visit where day(OrderDate) = day(now())-3')->num_rows;
$date_4 = new DateTime('-3 days'); $date_4 = $date_4->format('Y-m-d');
$visit_5 = $mysqli->query('select id from visit where day(OrderDate) = day(now())-2')->num_rows;
$date_5 = new DateTime('-2 days'); $date_5 = $date_5->format('Y-m-d');
$visit_6 = $mysqli->query('select id from visit where day(OrderDate) = day(now())-1')->num_rows;
$date_6 = new DateTime('-1 days'); $date_6 = $date_6->format('Y-m-d');
$visit_7 = $mysqli->query('select id from visit where day(OrderDate) = day(now())  ')->num_rows;
$date_7 = new DateTime('-0 days'); $date_7 = $date_7->format('Y-m-d');
?>
chart.data = [
<?echo '
{"date": "'.$date_1.'","value": '.$visit_1.'},
{"date": "'.$date_2.'","value": '.$visit_2.'},
{"date": "'.$date_3.'","value": '.$visit_3.'},
{"date": "'.$date_4.'","value": '.$visit_4.'},
{"date": "'.$date_5.'","value": '.$visit_5.'},
{"date": "'.$date_6.'","value": '.$visit_6.'},
{"date": "'.$date_7.'","value": '.$visit_7.'}
';?>
];
