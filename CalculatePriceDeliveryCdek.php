...
class CalculatePriceDeliveryCdek {
	...
	public function addTariffPriority($id, $priority = 0) {
        ...
        $priority = ($priority > 0) ? $priority : ( (is_array($this->tariffList))?count($this->tariffList)+1:1 );
		$this->tariffList[] = array( 'priority' => $priority,
									 'id' 		=> $id);
	}
	
	/**
	 * Получение массива заданных тарифов count($this->tariffList)+1
	 * 
	 * @return array
	 */
	...
	public function calculate() {
		...
		//город-отправитель
		isset($this->senderCityId) ? $data['senderCityPostCode'] = $this->senderCityId : '';
		//город-получатель
		isset($this->receiverCityId) ? $data['receiverCityPostCode'] = $this->receiverCityId : '';
		//выбранный тариф
