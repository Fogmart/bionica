...
if($_SESSION['store']!="1")exit();
					...
					if($_POST["about_post"]!=NULL)
					{
						...
						$mysqli->query("
						UPDATE `firma`
						SET `firma` = '".$_POST["sclad_post"]."',`about` = '".$_POST["about_post"]."'
					...
					{
					$mysqli->query("DELETE FROM `bionica-market`.`firma` WHERE `firma`.`id` = ".$_POST["id_del"].";");
...
if($_POST['name']!=NULL){
	...
	while($res = $data->fetch_assoc())
	{
		echo "<tr>";
			...
			echo "</td>";
			echo "<td style='height: 20px; vertical-align: top; border:1px solid black; background:#FFFFCB'>";
				echo '<input id="sclad'.$res['id'].'" name="sclad" value="'.$res['firma'].'">';
			echo "</td>";
			echo "<td style='height: 20px; vertical-align: top; border:1px solid black; background:#FFFFCB'>";
